...
int main()
{
	...
	*/
	camera = Camera(glm::vec3(0.0f, 10.0f, 10.0f), glm::vec3(0.0f, 5.0f, 0.0f), -60.0f, 0.0f, 0.15f, 0.5f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		modelaux = model;
		model = glm::scale(model, glm::vec3(2.0f, 2.5f, 3.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = modelaux;
		modelaux = modelaux2; //Se creo esta variable auxiliar 2 para iniciaizar una nueva matriz que nos lleve al origen
		                      //Y desacartar que se hereden las articulaciones
		model = modelaux;
