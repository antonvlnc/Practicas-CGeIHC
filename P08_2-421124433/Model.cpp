...
void Model::LoadMaterials(const aiScene* scene)
{
	...
	for (unsigned int i = 0; i < scene->mNumMaterials; i++)
	{
		...
		// Iterar sobre todos los tipos de texturas
		for (int type = aiTextureType_NONE; type <= aiTextureType_UNKNOWN; ++type)
		{
			if (material->GetTextureCount(static_cast<aiTextureType>(type)))
			{
				aiString path;
				if (material->GetTexture(static_cast<aiTextureType>(type), 0, &path) == AI_SUCCESS)
				{
					// Cargar la textura
		...
		// Si no se encontraron texturas, cargar una textura por defecto
		//if (!TextureList[i])
		//{
