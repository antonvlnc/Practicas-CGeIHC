...
void Mesh::CreateMesh(GLfloat* vertices, unsigned int* indices, unsigned int numOfVertices, unsigned int numberOfIndices)
{
	...
	glGenBuffers(1, &IBO); //IBO = arreglo de indices
	...
	glEnableVertexAttribArray(0);
	glBindBuffer(GL_ARRAY_BUFFER, 0); //Shader que recibe un variable de entrada
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0); //Shader que recibe dos variables de entrada
	glBindVertexArray(0);
...
void MeshColor::RenderMeshColor() //Función de Render (Dibujado)
{
	...
	glDrawArrays(GL_TRIANGLES, 0, vertexCount); //certexCount ya no se tiene que poner manualmente, ya está en una variable
	glBindVertexArray(0);
}
